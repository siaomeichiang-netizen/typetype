...
# --- 4. 生成按鈕與嚴格規範邏輯 ---
if st.button("🚀 啟動生成"):
    # 直接移除 for i in range(num_posts):
    with st.spinner("正在編寫極致去味文案..."):
        # 直接執行生成內容邏輯
        ...
        # 記得移除迴圈相關的縮排
    ...
    for i in range(num_posts):
        with st.spinner(f"正在編寫第 {i+1} 篇極致去味文案..."):
            ...
            【🎯 規則三：極致去標點符號 (真實感核心)】
            1. 嚴禁使用標準全形標點符號（例如：， 。 ？ ！ 、）。
            2. 80% 的斷句請強制使用「空格（空白鍵）」或「直接換行」來代替。
            3. 剩下的 20% 可以偶爾穿插「...」或是半形標點（如 , . ? ~）。
            4. 主文與底下的 10 則推文，全部都要嚴格遵守這個「空格斷句」的發文習慣。

            【❌ 規則四：絕對死線（只要踩到任何一條，該文案即視為徹底失敗）】
           ...
           【📋 規則五：推文輿論引導 (輿論佈局 - 精準格式版)】
            1. 輿論比例：10 則推文當中，必須有 7~8 則為「支持/分享/推薦」，其餘 2~3 則為「中性評論/負面體驗/詢問」。
            2. 嚴格格式規範：
               - 支持類內容，開頭必須強制使用「推|」。
               - 中性/負面/詢問類內容，開頭嚴禁使用任何符號（直接輸出文字）。
            3. 格式要求：每一則留言獨立成行，總共 10 則。
            4. 內容風格：
               - 支持類：包含具體細節（如術後效果、醫師手法）。
               - 中性/負面類：表現出理性提醒、路過詢問或客觀評價（如：痛感高、覺得沒預期好、問診所細節）。
            5. 嚴格遵守：禁止自創帳號、時間、IP。內容破碎隨性。

            【正確推文格式範例】
            推|我也覺得鳳凰打完效果有出來 但真的要選對診所
            推|術後多喝水差很多 我打完皮膚真的變亮
            打過一次覺得痛感比想像中高 大家真的要先諮詢
            推|這種儀器效果真的很看醫生技術+1
            好奇原PO是在哪一間諮詢的 諮詢費會貴嗎
            推|鳳凰打完真的有緊 但荷包很痛...
            我打完覺得效果普通 可能比較適合初期鬆弛
            推|我是去中山那間打的 覺得效果很穩
            推|推一個 這台對嘴邊肉真的有救
            推|我是搭配音波一起打 效果比單打更好
            """
